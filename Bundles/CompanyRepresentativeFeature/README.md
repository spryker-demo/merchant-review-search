...
company.representative.services.title,Customer service representative,en_US
company.representative.services.title,Kundenbetreuer,de_DE
...
use SprykerDemo\Yves\CompanyRepresentativeWidget\Widget\CompanyRepresentativeWidget;
...
class CompanyPageDependencyProvider extends SprykerCompanyPageDependencyProvider
{
    ...
    {
        return [
            CompanyRepresentativeWidget::class,
        ];
    }
}
```
...
```html
  {% if widgetExists('CompanyRepresentativeWidget') %}
    {% widget 'CompanyRepresentativeWidget' only %}{% endwidget %}
  {% endif %}
```
#### src/Pyz/Yves/ShopUi/Theme/default/components/atoms/icon-sprite/icon-sprite.twig
```svg
<symbol id=":company-representative" viewBox="0 0 33 33">
            <title id=":customer-representative">Company Representative</title>
